...
static int load_asset_config(asset_manager_ctx ctx) {
    ...
    cJSON_ArrayForEach(element, config_json) {
        ...
        a_info->asset_src = (char*) calloc(strlen(asset_src->valuestring) + 1, sizeof(char));
        if (a_info->asset_src == NULL) {
            log_error("Failed to allocate memory during parsing of asset config");
            free(a_info);
            return 1;
        }
        strcpy(a_info->asset_src, asset_src->valuestring);
        hashtable_set(ctx->assets, element->string, a_info);
        ...
        cJSON_ArrayForEach(texture_element, asset_textures) {
            ...
            if (texture_width == NULL || !cJSON_IsNumber(texture_width)) {
                log_error("Failed to parse asset config file: a texture's width must be a number");
                return 1;
            }
            cJSON *texture_height = cJSON_GetObjectItem(texture_element, "height");
            if (texture_height == NULL || !cJSON_IsNumber(texture_height)) {
                log_error("Failed to parse asset config file: a texture's height must be a number");
                return 1;
            }
            ...
            if (texture_offset_x == NULL || !cJSON_IsNumber(texture_offset_x)) {
                log_error("Failed to parse asset config file: a texture's offset_x must be a number");
                return 1;
            }
            ...
            if (texture_offset_y == NULL || !cJSON_IsNumber(texture_offset_y)) {
                log_error("Failed to parse asset config file: a texture's offset_y must be a number");
                return 1;
            }
            ...
            t_info->asset_id = (char*) calloc(strlen(element->string) + 1, sizeof(char));
            ...
            strcpy(t_info->asset_id, element->string);
